...
import fs from 'fs';
import { deleteVenteValidation } from '../../database/validations/demande/deleteVenteValidation.js';
...
const deletePreuve = async (demande) => {
    console.log("Suppression de la preuve du recu :", demande)
    try {
        if (!demande || !demande.preuve) return
        await fs.promises.unlink(path.join(UPLOADS_DIR, demande.preuve))
    } catch (error) {
        console.log("Error de suppression de la preuve de la demande :", demande)
    }
}
...
const getDemandes = async (req, res) => {
    ...
    try {
        const demandes = await prisma.demandeSuppressionVente.findMany({
...
const createDemande = async (req, res) => {
    ...
    await prisma.$transaction(async (tx) => {
        try {
            // validation
            const last = await tx.demandeSuppressionVente.findFirst({
                orderBy: { id: 'desc' },
                select: { id: true }
            });
            const resultDemande = deleteVenteValidation.safeParse({ ...req.body, code: `DSV-00${last?.id ? (last?.id + 1) : 1}` });
            ...
            const newDemande = await tx.demandeSuppressionVente.create({
                data: {
                    ...resultDemande.data,
...
const updateDemande = async (req, res) => {
    ...
    await prisma.$transaction(async (tx) => {
        try {
            // found
            const demandeFound = await tx.demandeSuppressionVente.findFirst({
                where: { id: parseInt(id), deletedAt: null }
            })
            if (!demandeFound) return res.status(400).json({ error: "Cette demande de suppression de vente n'existe pas!" })
            ...
            const resultDemande = deleteVenteValidation.safeParse({
                ...demandeFound,
                ...req.body,
            })
            ...
            // modification de la demande de suppression de vente de la base de données et log du résultat
            const updatedDemande = await tx.demandeSuppressionVente.update({
                where: { id: parseInt(id), deletedAt: null },
                data: {
                    ...resultDemande.data,
                    preuve: req.file?.filename,
...
const validerDemande = async (req, res) => {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        try {
            let demandeFound = await tx.demandeSuppressionVente.findUnique({
                where: { id: parseInt(id), deletedAt: null }
            });
            ...
            await tx.demandeSuppressionVente.update({
                where: { id: parseInt(id), deletedAt: null },
                data: {
...
const deleteDemande = async (req, res) => {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        try {
            ...
            // suppression de la demande de suppression de vente de la base de données et log du résultat
            await tx.demandeSuppressionVente.update({
                where: { id: parseInt(id), deletedAt: null },
